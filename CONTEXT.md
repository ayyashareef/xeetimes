...
- **`git push` to `main` no longer deploys anywhere.** `.github/workflows/deploy.yml`'s push trigger was **removed 2026-07-18** (it targeted the **old hirinews droplet** — `DEPLOY_HOST` secret → `/var/www/hirinews`, beta.hirinews.com — never this VM); the workflow is now manual-only (Actions tab → Run workflow). The live beta is still updated **manually** on the `168.144.96.165` VM (see Deploying above). Pushing is now safe for a plain GitHub sync.
