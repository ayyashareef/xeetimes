...
export default async function CategoryPage({
  ...
  // The lead row (carousel + the featured article beside it) is a SHOWCASE of the
  // newest articles, not a slice taken out of the list: the same articles appear
  // again in the grid below, so nothing is only reachable via the carousel.
  // Pagination therefore counts the grid alone — a plain 8 per page.
  const page = Math.min(rawPage, Math.max(1, catPageCount(total)));
  const skip = (page - 1) * CAT_PER_PAGE;
  const take = CAT_PER_PAGE;
  ...
  const cp: CatPage = {
    ...
    mostRead: [],
    // Full page of articles — including the ones showcased in the lead row above.
    grid: enEmpty ? [] : articles,
  };
