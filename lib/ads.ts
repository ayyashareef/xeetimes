...
export async function getActiveAds(): Promise<AdsMap> {
  ...
  const ads = await db.advertisement.findMany({
    ...
    select: { id: true, imageUrl: true, linkUrl: true, title: true, slot: true, rotateSeconds: true, categorySlug: true, sponsorLabel: true },
  });
  // Group every active ad by slot (multiple ads per slot rotate on the client).
  const map: AdsMap = {};
  for (const a of ads) {
    (map[a.slot] ||= []).push({ id: a.id, imageUrl: a.imageUrl, linkUrl: a.linkUrl, title: a.title, rotateSeconds: a.rotateSeconds || 6, categorySlug: a.categorySlug, sponsorLabel: a.sponsorLabel });
  }
  return map;
}
