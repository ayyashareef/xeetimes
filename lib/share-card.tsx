...
/**
 * The article share card.
 *
 * `withLatin` decides whether the romanised headline is printed under the
 * Thaana one. It is a PARAMETER, not something sniffed from the user agent:
 * Facebook and X were handed the same image URL, so the version each got
 * depended on which agent happened to fetch that URL — and X does not fetch
...
export async function renderShareCard({ id, withLatin }: { id: string; withLatin: boolean }) {
  ...
  const latinRaw = !withLatin ? '' : (article?.metaDescription_dv || article?.excerpt_dv || '').trim();
